...
好的图表应该有以下几个要素：
** 可读性、精准性、客观性、统一性 **   
可读性：图表首要功能是解释不是设计，切勿陷入设计过度的陷阱；好的图表要解释数据，学会选用正确地图表表达数据。  
精准性：为了使数据的解读不失真，数据应要求精确到小数位；注意单位换算造成的数据失真。  
客观性：数据的解读因为每个人的观点和视角不同，可以呈现诸多结果，有经验的数据分析师能够引导分析的结果。  
统一性：统一是规范和约束图表，当图表过多时，一个统一标准不会让人眼花缭乱。  

配色建议：[Color wheel | Color schemes - Adobe Color CC](https://color.adobe.com/create/color-wheel/)
